...
class TokenRepository(BaseTokenRepository):
    ...
    def get_by_id(self, id: int, token_is_active: int = 1):
        result = self._session.execute(
            text(
                "SELECT * FROM tokens WHERE token_id = :id AND token_is_active = :token_is_active"
            ),
            {"id": id, "token_is_active": token_is_active},
        )
    ...
    def get_by_user_id(self, user_id: int, token_is_active: int = 1):
        result = self._session.execute(
            text(
                "SELECT * FROM tokens WHERE token_user_id = :user_id  AND token_is_active = :token_is_active"
            ),
    ...
    def get_token_by_username_and_value(
        ...
        result = self._session.execute(
            ...
            ),
            {
                "user_username": user_username,
                "token_value": token_value,
                "token_is_active": token_is_active,
            },
        )
        row = result.mappings().one()
        if row:
